...
import com.pawelzabczynski.security.auth.AuthService.{
  AuthError,
  AuthInternalError,
  ExpiredToken,
  NotExists,
...
class Auth[A](authOps: AuthOps[A], clock: Clock, xa: Transactor[Task])
    extends Service {
  override def auth(id: Id): IO[AuthService.AuthError, User] = {
    for {
      maybeToken <- authOps
        ...
        .tapError(e =>
          ZIO.logErrorCause(
            s"Error occurred when try find token for ID: `$id`",
            e.toCause
          )
        )
      ...
      maybeUser <- UserModel
        ...
        .tapError(e =>
          ZIO.logErrorCause(
            "Error occurred when try fetch user information",
            e.toCause
          )
        )
    ...
    clock.now[UIO].flatMap { ts =>
      if (ts.isAfter(authOps.validUntil(token)))
        authOps
          ...
          .tapError(e =>
            ZIO.logErrorCause(
              s"Error occurred when try delete token.",
              e.toCause
            )
          )
...
object Auth {
  ...
  val liveApiKeyAuth
      : URLayer[AuthOps[ApiKey] with Clock with Transactor[Task], Auth[
        ApiKey
      ]] =
